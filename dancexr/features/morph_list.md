...
The Morph List appears in the actor menu of any loaded **PMX** model. XPS models use the [Dressing system](/dancexr/features/optionals) instead.
...
- **Material morphs** — color or transparency shifts (used by the [dressing system](/dancexr/features/optionals))
...
Morphs stay applied until you change them or reload the model. Some morphs (like facial expressions) will be overridden by [Facial control](/dancexr/features/facial_control) and [Blink, breathing & eye contact](/dancexr/features/eyecontact) when those are active.
...
- **Material morphs** showing up here are typically the same ones surfaced by the [Dressing system](/dancexr/features/optionals); applying from either place has the same effect.
