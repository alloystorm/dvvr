---
layout: home
title: DanceXR — どんなモデルでも、どこでもアニメーション
toc: false
locale: ja-JP
...
ほとんどのツールでは、すべてのモデルとモーションの組み合わせに対して、手動でのボーン修正、ポーズ調整、設定の微調整が必要です。DanceXRはそれを自動的に処理します。

- **ユニバーサルモーションの互換性** — Tポーズ用でもAポーズ用でも、VMDおよびBVHモーションはどのモデルでも正しく再生されます
- **オンザフライのボーン修正** — 一般的なボーン構造の問題は自動的に修正されます
- **PMXとXPSの完全サポート** — 両方のフォーマットの物理演算、マテリアル、ポーズをサポート
...
DanceXRのキャラクターはただ立っているだけではありません。設定なしで、呼吸し、瞬きし、アイコンタクトを取ります。モデルをロードした瞬間からすでにそこにいるように感じられます。

- アクターはあなたが視界に入るとあなたを見つめ、複数のキャラクターがステージを共有しているときはお互いを見つめます
- どんなモーションの上にもレイヤー化されるプロシージャルな呼吸 — 何をしていても常にアクティブ
- **オートダンス** — 音楽のビートと音量からリアルタイムで振り付けを生成します
- キャットウォーク、アイドルモーション、モーションシーケンスが、手動の設定なしでシーンをダイナミックに保ちます
...
DanceXRには、髪、服、ソフトボディ、スカートがすべてモーションに反応するリアルタイムのフル物理シミュレーションスタックが含まれています。

{:.section-label}
物理演算

- **PMXモデルの物理演算を完全サポート** — PMXモデルの物理コンポーネントは自動的に適用され、強力な物理エディターを使用してモデルごとにカスタマイズできます
- **髪、スカート、胸の物理演算** — XPSモデル用の簡単な設定で、数回のクリックで自然な物理シミュレーションを実現します
...
- **布シミュレーション** — 破れ、ドラッグ、メッシュから布への変換を備えたリアルタイム並列スレッド布シミュレーション
- **XPBDパーティクルダイナミクス** — 最高の効率を達成するために並列で実行されるカスタムビルドの高度なパーティクルベースシミュレーション
- **ソフトボディシミュレーション** — XPBDを使用してXPS物理ツールをさらに強化
</div>
...
{:.section-label}
雰囲気
- **パーティクル効果** — 雪、きらめき、雨、落ち葉を完全な物理制御で
- **プロシージャルスカイ** — ダイナミックな昼夜の移行、星、風に影響されるボリュメトリッククラウド
...
{:.section-label}
環境
- **HDR環境マップ** — リアルな照明と背景に高ダイナミックレンジ球状環境マップを使用
- **リアルタイムウォーターシステム** — 反射、屈折、波紋、水中物理
- **ステージモデル、プロップ、鏡、レーザー、および完全なウォーターシステム**
</div>
...
あなたが望むショットを正確にキャプチャするための複数のカメラモード。

- **オートカメラ** — すべてのアクターを自動的にフレーム内に保持するスマートビューフレーミング
- **フリーフライ、オービット、一人称、固定（コンサートモード）、ワンショット**
- VMDファイルからのカメラモーション（補間カーブの完全サポート付き）
...
## あらゆるプラットフォームで素晴らしいビジュアル

ハードウェアに合ったビルドを選択してください：

| ビルド | フォーカス |
|-------|-------|
| **RT** | レイトレーシング — 反射、グローバルイルミネーション、アンビエントオクルージョン、コンタクトシャドウ |
| **HD** | 高い視覚的品質、バランスの取れたパフォーマンス |
...
AIパワードのボイスチャットにより、キャラクターがリアルタイムで応答します。

- **OpenAI**、ローカルLLM（LM Studio、Ollama、Ooba）、またはリモートGPUサービスに接続
- 900以上の音声を備えた**内蔵TTS** — 各キャラクターが独自の音声を持てます
- **自動リップシンク** — 話すときに顔がアニメーション化します
...
**Creator Edition**は、録画を画面やフレームレートから分離します。

- そのレートをライブで維持できない1080pモニターでも、4K動画を60fpsでレンダリングします
...
<div class="release-card">
  ...
  <div class="release-items">
    <div class="release-item">継続するAIロールプレイ</div>
    <div class="release-item">OperatorXR ワークフロー統合</div>
