...
### 移動
アクターの選択ディスクがあります。UIが表示されている場合、ディスクも表示されるため、ディスクをドラッグしてアクターを移動できます。

デスクトップモードではマウスでドラッグできます。VRモードではポインターでドラッグできます。

### 回転と垂直移動
アクターをドラッグしている間、マウスホイールまたはVRコントローラーのサムスティックを左右に動かしてアクターを回転させることができます。

マウスに水平スクロールホイールがある場合は、それを使用してアクターを上下に移動させることができます。

VRモードでは、サムスティックを上下に動かしてアクターを垂直に移動させます。

### ギズモキューブ
ギズモキューブは、アクターを移動したりポーズさせたりするために使用できる仮想的なキューブです。サポートされているモーションまたはツールがアクティブになると表示されます。現在、以下のモーションとツールがギズモキューブをサポートしています。
...
* [セックスモーション3](sex_motion_3){: .nsfw}
* モーションオーバーライド

ギズモキューブが表示されている場合は、マウスまたはポインターをその面のいずれかに置き、表面上をドラッグして移動し、ホイールまたはサムスティックを使用して表面内で回転させます。

通常、手には2つのキューブ、足には2つのキューブ、胴体には1つのキューブがあります。したがって、これらのボディパーツを個別に移動およびポーズを調整することができます。
...
以下のリンクは、各設定の詳細なドキュメントへのリンクです。
* モーション割り当てと設定
* 最近変更された設定
* [ドレッシングシステム（マテリアルモーフとXPSオプショナル）](optionals)
* [ボーンマッパー（XPSのみ）](bone_mapper)
