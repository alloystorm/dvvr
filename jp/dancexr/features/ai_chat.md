...
**注意:** アシスタントまたはチャット機能を使用するには、DanceXR Operator バージョン 1.1 が必要です。[こちらから無料でダウンロード](https://github.com/alloystorm/dvvr/releases/download/operator1.0/operator-installer-lite-1.1.262.exe)できます。
...
**設定:**
* DanceXR フォルダーの横に Operator をインストールします。詳しい手順は [DanceXR Operator](operator) の機能ページを参照してください。
...
### Operator アシスタント
2026.6 リリースで導入された Operator アシスタントは、チャットウィンドウ内でそのまま DanceXR に関する質問に答えてくれる組み込みのヘルパーです。**@operator** に続けて質問を入力すると、関連するドキュメントを調べ、分かりやすい言葉で回答し、さらに読み進められるようにドキュメントへのクリック可能なリンクを添えてくれます。キャラクターを読み込む前でも動作するため、いつでもヘルプを求められ、ロールプレイの履歴とは別に独自の会話スレッドを保持します。

2 つの手軽なスラッシュコマンドで、会話を離れずにセットアップできます。

* **`/scene <description>`** — 短いテキスト説明から現在のシーンをセットアップまたは再生成します。
* **`/char <name> <description>`** — 名前と説明からキャラクターのプロフィールを再生成します。よく知られたキャラクターであれば、アシスタントが Web を検索して詳細を補完できます。

2026.6 では会話もより堅牢になりました。空の応答や途中で途切れた応答は流れを乱さずスムーズに処理され、メッセージ内のリンクはクリックできるようになり、ウェルカムメッセージとより分かりやすい読み込み・進捗の表示によって、アシスタントが何をしているのか把握しやすくなりました。
