...
**2024.1** に追加されました。`[content library](../preparecontent)` 内の `presets/` に保存されるため、プリセットは同じ DanceXR バージョンのユーザー間で共有できます。
...
非常に異なるモデル間では、ボーン名に依存する設定（XPS物理リグ、[ボーンマッパー](bone_mapper) のオーバーライドなど）は、クリーンに引き継がれない場合があります。
...
- [シーンを保存](save_scene) — 単一のアクターの設定ではなく、シーン全体をキャプチャします
- [シーンバンドル](scene_bundle) — 保存されたシーンに、その依存アセットをパッケージ化します
- [コンテンツライブラリ](../preparecontent) — `presets/` フォルダーの場所
