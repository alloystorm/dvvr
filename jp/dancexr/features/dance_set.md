...
リミックスは、DanceXRでは、1つのダンスセットからのモーションデータを別のダンスセットの音声と組み合わせることを意味します。この機能を使用すると、異なる音声に対してモーションを適応することができます。また、モーションと音楽の速度を自動的に調整します。

## VMD2PNG (v2026.3)
[VMD2PNG](https://github.com/alloystorm/vmd2png) は、VMD モーションデータを PNG 画像ファイルにエンコードするオープンソースツールで、より小さなファイルサイズ、簡単な共有、およびモーションデータの可視化を提供します。

DanceXR 2026.3 は VMD2PNG ファイルからモーションを読み込むことをサポートしています。PNG ファイルをアプリケーションのウィンドウにドラッグ＆ドロップするか、コンテンツライブラリに他のモーションファイルと一緒に PNG ファイルを配置することで読み込むことができます。