...
## モーションスムージング（v2026.2）
DanceXR 2026.2では、物理ベースのモーションスムージングアルゴリズムが導入され、リアルな加速度とダンピング効果をシミュレートして自然なモーションダイナミクスを再現する滑らかさを向上させています。

すべてのモーションに対してスムージングはデフォルトで有効になります。完全に無効にすることもでき、個々の関節のスムージングパラメータを調整してご希望に合わせることができます。以前のアルゴリズムと比較して、このバージョンではよりレスポンス性のある体験を実現するためのレイテンシーを削減しています。