...
support_sections:
  - id: known-issues
    ...
    subsections:
      - title: 📌 バージョン 2026.6
        items:
          - question: モーション設定で T ポーズまたはカスタムリギングを使用した場合、モーションが壊れる（上半身が動かない、またはねじれる）。または、指のモーションが正しく適用されない。
            answer: |
              これはモーションスムージングシステムのバグが原因です。一時的な回避策として、モーション設定でモーションスムージングを無効にしてください。
      - title: 📌 バージョン 2026.5 - 2026.6
        items:
          - question: 特定のモデルとモーションで脚が固定位置に向かって引かれている
            answer: |
              これは、モーションからIKの動きがない場合にモデル定義のIKが有効になっているためです。回避策：アクターモーション設定から「モデルIK」を無効にしてください。 2026.7リリース以降は不要です。
  - id: faq
    label: よくある質問
    title: よくある質問（FAQ）
    subsections:
