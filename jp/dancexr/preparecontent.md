...
* texture
  ...
  * drawing: [ボディペイント機能](features/outfit_body_paint)用の保存された画像
  * ground: 地面のテクスチャ
  * mask: モデルに適用できる[詳細＆法線マップ](features/custom_detail_map)
  * particle: [パーティクルエフェクト](features/particles)用のテクスチャ
...
* bundles: [必要なすべてのアセットが含まれた保存されたシーン](features/scene_bundle)を含むzipパッケージ
...
* videos: [投影およびダイナミックテクスチャマップ](features/video_playback)に使用できるビデオ。MP4形式のみをサポート。
...
DanceXRは[Google Driveからファイルをダウンロード](features/googledrive)することができます。ドライブフォルダが制限なく共有されている限り、共有フォルダのURLを入力するだけで、DanceXRはドライブフォルダをスキャンし、ローカルに存在しないファイルをダウンロードすることができます。
