...
내장 프리셋은 한 번의 탭만으로 전체 작동하는 모습을 구현하며, 모든 매개변수는 이후 자유롭게 조정할 수 있습니다.


## 프리셋

내장 프리셋(*일몰*, *주광*, *창문*, *무대*, *프로젝터* 등)은 단 한 번의 탭만으로 완벽한 조명 설정을 확립합니다. 가장 유사한 프리셋을 적용한 후 개별 그룹을 조정하세요.
...
**추가 1**, **2**, 및 **3**은 독립적이며 완전히 구성 가능한 조명 그룹입니다. 일반적으로 키(key), 채움(fill), 그리고 림(rim) 조명으로 사용됩니다. 각각은 스포트라이트, 포인트 라이트, 영역 라이트 또는 프로젝터일 수 있으며, 액터를 동적으로 추적할 수 있습니다.
