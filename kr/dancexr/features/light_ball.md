...
**Style** 패널은 프리셋(Glow, Reflective, Crystal)과 색상, 광택, glow, 메탈릭, 강도, 굴절, 그림자, 트레일을 사용하여 모양을 제어합니다. **Stage Color**를 사용하여 씬 조명과 동기화하거나 **Flashing With Beats**를 사용하여 오디오 반응성으로 맥동하게 만듭니다.
...
오디오 반응성 요소에 대한 기본 색상과 발광 강도를 담고 있습니다. Glow는 색상과 곱해지며, 자동 업데이트가 활성화되면 비트에 맞춰 애니메이션됩니다.

## Physics

라이트 볼 물리 동작에 대한 중첩 구성입니다. 프리셋: **Spring** (중력/충돌 없음), **Hanging** (중력 + 충돌), **Floating** (중력/충돌 없음). **Gravity**는 하방 힘을 활성화합니다. **Collision**은 물리 충돌을 활성화합니다. **Mass**, **Distance**, **Spring Force**, 및 **Damping**은 스프링 관절을 구성합니다. **Min/Max Speed**는 속도를 제한합니다.