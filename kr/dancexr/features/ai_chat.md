...
**설정:**
* DanceXR 폴더 옆에 Operator를 설치합니다. 설치 방법은 [DanceXR Operator](operator) 기능 페이지를 참고하세요.
...
### Operator 어시스턴트
2026.6 릴리스에서 도입된 Operator 어시스턴트는 채팅 창 안에서 바로 DanceXR에 관한 질문에 답해 주는 내장 도우미입니다. **@operator** 뒤에 질문을 입력하면 관련 문서를 찾아보고, 쉬운 말로 답하며, 더 읽어 볼 수 있도록 문서로 연결되는 클릭 가능한 링크를 함께 제공합니다. 캐릭터를 불러오기 전에도 작동하므로 언제든지 도움을 청할 수 있고, 자신의 대화 스레드를 롤플레이 기록과 별도로 유지합니다.

두 가지 간단한 슬래시 명령으로 대화를 떠나지 않고도 설정할 수 있습니다.

* **`/scene <description>`** — 짧은 텍스트 설명으로 현재 장면을 설정하거나 다시 생성합니다.
* **`/char <name> <description>`** — 이름과 설명으로 캐릭터 프로필을 다시 생성합니다. 잘 알려진 캐릭터라면 어시스턴트가 웹을 검색해 세부 정보를 채울 수 있습니다.

2026.6에서는 대화도 더 견고해졌습니다. 빈 응답이나 중간에 끊긴 응답도 흐름을 끊지 않고 매끄럽게 처리되며, 메시지의 링크를 클릭할 수 있게 되었고, 환영 메시지와 더 명확한 로딩 및 진행 표시 덕분에 어시스턴트가 무엇을 하고 있는지 파악하기 쉬워졌습니다.
