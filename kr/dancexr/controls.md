...
VR 전용 작동(핸드 컨트롤러, 포인터 보정, 편안함, 포인터 핸들, 마우스-VR)에 대해서는 [VR operations](vr_operations) 페이지를 참조하십시오.
...
- [Wind field in sky settings](features/sky#wind_field): 바람장 위치와 방향을 변경합니다.
- [Various procedural motions]: 몸통, 손, 발과 같은 신체 부위의 오프셋 및 회전을 변경합니다.
- [Motion Override](features/motion_override): 오버라이드된 모션의 신체 부위 위치와 회전을 조정합니다.
...
AI 채팅을 위한 마이크 토글(기본값: 오른손 컨트롤러 메뉴 버튼)도 여기서 구성할 수 있습니다 — [AI Voice Chat](features/ai_chat#key-binding)을 참조하십시오.

---

## 추가 읽을거리

- [VR operations](vr_operations) — VR 전용 상호작용 (핸드 컨트롤러, 포인터, 그립 드래그 UI, 편안함)
- [VR settings](features/vr_settings) — VR 기술 설정 (피포리티드 렌더링, 포인터 보정, 핸드 렌더링)
- [Concepts & glossary](concepts) — 선택 디스크, 기즈모 큐브, 토글 상태의 정의
- [Getting started](getting-started) — 첫 실행 안내