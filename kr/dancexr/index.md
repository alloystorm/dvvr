...
- **천 시뮬레이션** — 찢기, 드래그 및 메쉬-천 변환이 있는 실시간 병렬 스레드 천 시뮬레이션
- **XPBD 파티클 다이내믹스** — 최고의 효율성을 달성하기 위해 병렬로 실행되는 맞춤형 고급 파티클 기반 시뮬레이션
- **소프트 바디 시뮬레이션** — XPBD로 XPS 물리 도구를 더욱 강력하게 향상
</div>
...
{:.section-label}
분위기
- **파티클 효과** — 완벽한 물리 제어가 가능한 눈, 반짝임, 비, 떨어지는 나뭇잎
- **절차적 하늘** — 역동적인 주야간 전환, 별, 바람의 영향을 받는 볼류메트릭 구름
...
{:.section-label}
환경
- **HDR 환경 맵** — 사실적인 조명 및 배경을 위해 하이 다이내믹 레인지 구형 환경 맵을 사용하세요
- **실시간 물 시스템** — 반사, 굴절, 잔물결 및 수중 물리
- **무대 모델, 소품, 거울, 레이저 및 전체 물 시스템**
</div>
...
원하는 샷을 정확히 포착하기 위한 다양한 카메라 모드.

- **자동 카메라** — 모든 액터를 프레임 안에 자동으로 유지하는 스마트 뷰 프레이밍
- **프리플라이, 궤도, 1인칭, 고정(콘서트 모드), 원샷**
- 보간 곡선 지원을 갖춘 VMD 파일의 카메라 모션
...
| 빌드 | 중점 |
|-------|-------|
| **RT** | 레이 트레이싱 — 반사, 글로벌 일루미네이션, 앰비언트 오클루전, 접촉 그림자 |
| **HD** | 높은 시각적 품질, 균형 잡힌 성능 |
| **LW** | 최적화된 프레임 속도 — 모바일 및 Quest에 최적 |
...
AI 기반 음성 채팅을 통해 캐릭터가 실시간으로 응답합니다.

- **OpenAI**, 로컬 LLM(LM Studio, Ollama, Ooba) 또는 원격 GPU 서비스에 연결
- 900개 이상의 음성이 있는 **내장 TTS** — 각 캐릭터가 자신만의 음성을 가질 수 있습니다.
- **자동 립싱크** — 말할 때 얼굴이 애니메이션됩니다.
...
**Creator Edition**은 녹화를 화면이나 프레임 속도에서 분리합니다.

- 실시간으로 그 속도를 유지할 수 없는 1080p 모니터에서도 60fps로 4K 비디오 렌더링
