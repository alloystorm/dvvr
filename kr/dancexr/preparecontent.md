...
* bundles: [필요한 모든 에셋이 포함된 저장된 씬](features/scene_bundle)을 포함한 zip 패키지
...
DanceXR은 [Google 드라이브에서 파일을 다운로드](features/googledrive)할 수 있습니다. 드라이브 폴더가 제한 없이 공유되어 있다면 가능합니다. 공유 폴더의 URL을 입력하면 DanceXR이 드라이브 폴더를 스캔하고 로컬에 존재하지 않는 파일을 다운로드할 수 있습니다.
