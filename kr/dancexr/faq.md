...
모든 것이 필요할 것은 활성화 단계를 다시 수행하는 것입니다. 그러면 정상적으로 작동해야 합니다. 이와 관련하여 문제가 발생하는 경우 알려주시면 기꺼이 도와드릴 수 있습니다.
...
* SteamVR을 사용하는 경우 헤드셋이 연결된 상태에서 SteamVR을 열고 작은 SteamVR 창에서 왼쪽 상단을 클릭한 다음 "설정"을 선택한 다음 "개발자" 섹션으로 이동하여 "현재 OpenXR 런타임" 옵션을 볼 수 있습니다. 아래 버튼을 클릭하여 "SteamVR을 OpenXR 런타임으로 설정"하세요.
...
기본적으로 투명도 깊이 사전 패스가 켜져 있습니다. 이는 투명한 재료를 렌더링하기 전에 깊이 패스를 실행하여 맨 위에 있는 것들만 선택하여 렌더링함으로써 투명도 정렬 문제를 해결합니다. 이 기술의 부작용은 여러 개의 투명한 재료가 서로 겹쳐져 있을 때, 맨 위에 있는 것만 렌더링된다는 것입니다.
