...
- **앱이 실행되지 않거나 시작 시 충돌함** → [FAQ → 스카이만 표시됨](faq#only-sky-is-displayed-no-ui-or-camera-control-available), [FAQ → 갑자기 충돌하기 시작함](faq)
- **VR이 시작되지 않음** → [FAQ → VR 실행 불가](faq#unable-to-launch-vr)
- **활성화 문제** → [FAQ → 다시 활성화하라는 요청](faq#im-asked-to-activate-again), 그런 다음 [지원](support)
- **모델별 문제 (특정 모델의 오작동)** → [액터 문제 해결](features/troubleshooting)
- **머리카락, 천, 치마 또는 가슴 물리 효과** → [물리 시스템](physics)
- **재질 또는 텍스처가 잘못 보임** → [외관 및 재질](appearance), [FAQ → 모델은 로드되었으나 모든 것이 하얗게 보임](faq#model-loads-but-everything-is-white)
- **모션이 재생되지 않거나 잘못된 액터임** → [모션 시스템](motion)
- **용어의 의미를 모를 때** → [개념 및 용어집](concepts)

## 관련 페이지

- [FAQ](faq)
- [지원](support) — Discord, 이메일, GitHub 이슈
- [개념 및 용어집](concepts)