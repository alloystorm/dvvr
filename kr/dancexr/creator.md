...
크리에이터 에디션은 오프라인 렌더링 기능을 제공하여 실제 프레임 속도와 화면 해상도에 대해 걱정하지 않고 비디오를 녹화할 수 있습니다. 예를 들어, 모니터가 1080p이고 컴퓨터가 해당 프레임 속도를 유지할 수 없을 때에도 4K 비디오를 매끄럽게 60fps로 녹화할 수 있습니다. 또한 3D 및 VR 180 비디오도 생성할 수 있습니다.
...
3D SBS의 경우, 출력 생성을 위해 2개의 카메라가 사용되며, 이미지는 나란히 배치됩니다.

VR 180의 경우, 이것이 실제 180도 시야각을 제공하지 않음에 유의하십시오. 대신 각 카메라는 120도에서 렌더링되며 외부 영역은 검은색으로 채워집니다. 이것은 현재 효율성을 위한 타협입니다. 앞으로의 업데이트에서 전체 180도 및 360도 시야각 VR 비디오를 제공하기 위한 옵션을 평가할 것입니다.

2D 및 3D SBS 모드는 설정에서 선택한 해상도를 사용하며, VR180 출력은 4096x2048로 고정되어 있고, 각 눈은 2048x2048입니다.
...
이미지를 비디오로 변환하려면 ffmpeg와 같은 도구를 사용해야 합니다. (비디오 편집에 익숙하다면 선호하는 도구를 사용할 수 있습니다. 여기서는 사용 편의성 때문에 FFMPEG만 권장합니다.)

Windows에서 ffmpeg를 설치하는 방법에 대한 글입니다: https://www.wikihow.com/Install-FFmpeg-on-Windows

ffmpeg를 설치하고 환경 변수에 경로를 추가하면 사용할 수 있습니다.

출력 이미지가 포함된 폴더에는 시스템에 ffmpeg가 설치되어 있다면 변환을 수행하는 배치 파일도 있습니다.
...
이 명령어는 movie_####.jpg와 sound.wav를 입력으로 사용하여 30fps의 비디오를 생성하고 출력을 movie.mp4로 저장합니다.
...
**"-i sound.wav"**는 오디오 파일의 경로입니다. 비디오에 오디오가 필요하지 않은 경우 이 부분은 생략할 수 있습니다.
...
**VR180** 비디오의 경우, 비디오가 Youtube와 같은 플랫폼에서 올바르게 인식되도록 올바른 태그를 설정하기 위해 다른 도구가 필요합니다. 이 단계에서 권장되는 도구는 Google의 "VR180 Creator"입니다.
