...
**設置：**
* 將 Operator 安裝到您的 DanceXR 資料夾旁邊。詳細安裝方式請參考 [DanceXR Operator](operator) 功能頁。
...
### Operator 助手
Operator 助手在 2026.6 版本中引入，是一個內建助手，可在聊天視窗內直接回答你關於 DanceXR 的問題。輸入 **@operator** 後接一個問題，它就會查閱相關文件，用淺顯的語言作答，並附上指向文件的可點擊連結，方便你進一步閱讀。它甚至在你尚未載入角色時也能運作，因此你隨時都可以求助，而且它會將自己的對話串與你的角色扮演歷史分開保存。

兩個便捷的斜線指令讓你不必離開對話即可完成設定：

* **`/scene <description>`** — 根據一段簡短的文字描述設定或重新生成目前場景。
