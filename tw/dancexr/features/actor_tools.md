...
## 工具選單
當您點擊選擇圓盤時，演員選單會彈出。在演員名稱旁邊，有一個「螺絲刀和錘子」圖標，會打開演員工具選單。

* **收藏** - 將模型加入或移出收藏列表
* **標籤** - 修改模型的標籤
* **旁觀者** - 切換模型的旁觀者模式
* **上移** - 在列表中將模型上移。這會影響它們在陣容中的位置，並且如果啟用自動分配動作，會更改分配的動作。
* **下移** - 在列表中將模型下移。這會影響它們在陣容中的位置，並且如果啟用自動分配動作，會更改分配的動作。
* **重置位置** - 將演員重置為預設位置和旋轉
* **重置物理** - 重置演員的物理元件
* **重置所有設定** - 重置演員的所有設定
* **複製** - 複製演員
* **重新載入** - 重新載入模型
* **3D快照** - 將模型在目前姿勢下轉換為OBJ格式的3D模型檔案
* **移除** - 移除演員
...
* Pro
    ...
    * [乳搖和性愛覆蓋](boob_shake_sex_overlay){: .nsfw}
    * [假陽具](dildo){: .nsfw}
    * 光球
