...
內建預設值（例如 *日落*、*日光*、*窗戶*、*舞台*、*投影機* 等）可以在一鍵內建立完整的照明佈局。請先套用最接近的預設值，再從中調整各個獨立光源組。
...
**天體**子區塊控制方向性太陽（以及 HDRP 中的月亮）。一天中的時間、朝向和黃道角度決定了太陽在天空的位置以及由此產生的陰影方向。
...
控制方向性太陽（以及 HDRP 中的月亮和夜空）。太陽位置由一天中的時間、朝向和黃道角度定義，可為陰影方向和天空顏色提供完整的創作控制。
...
**窗戶**子區塊會投射出模擬穿過窗格的矩形陰影網格。使用「位置」、「底部」和「朝向」設置窗口，用「寬度」、「高度」、「行數」和「列數」確定尺寸，而上方太陽光控制則決定了光束通過它們的方向。*天光* 選項會增加來自相同方向的柔和補光，以補充陰影。
...
**類型**選擇光源形狀：聚光燈、點光源、區域光源、金字塔或箱體投影儀。**紋理**會在光束中投射圖案（窗戶、百葉窗、光點、管狀、視頻）。設定**發射器半徑**以柔化錐體或紋理的邊緣，並使用 **可見** 來控制光源本身在渲染中看起來的亮度。


### 位置與朝向


**距離**和**高度**將光源放置在相對於其目標物的位置，**角度**向下傾斜它，而**朝向**則沿垂直軸旋轉它。對於聚光燈類型，**大小 X / Y**會擴大光束的橫截面；**錐體長度**控制體積散射深度。


### 動態 (Dynamics)


**動態**決定光源是保持固定（*靜止*）、圍繞指定的演員運行（*跟隨演員* / *後方演員*），還是沿著設定的半徑移動（*保持距離*）。啟用 **使用演員位置** 可以將光源的方向性設定為相對於演員面對的方向。演員指派由上層照明面板中的分配設定處理。
