...
拖曳手/臂或腳/腿來移動該肢體；抓取動作會在固定深度的指標光束上運行，因此移動攝影機（或滾動滑鼠）會使其前後掃描變動。目標點會被限制在肢體的自然活動範圍內，所以無法塌陷到關節處或鎖死成完全直線。手部會在釋放時保持原位。腳部使用 *持續定點*：持續拖曳直到標記變成綠色為止才能將腳固定在空中；或者，在它仍紅色時釋放以讓腿落下來，穩固於地面。**重設（Reset）**動作會解除所有定點，並讓角色恢復到中立的站姿。
