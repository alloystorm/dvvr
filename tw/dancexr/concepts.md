...
**角色（Actor）**
：載入場景的角色的模型。角色來自 PMX 或 XPS 檔案。每個角色都有其自身的動態、材質、物理和行為設定。多個角色可以共用一個舞台；每個角色腳下的**選取圓盤**是您選擇對象的方法。參閱 [角色菜單與工具](/dancexr/features/actor_tools)。
...
**菜單欄（Menu bar）**
：屏幕底部（或在 VR 中漂浮在您前方）的圖標條。左側的五個圖標分別開啟了系統、環境、場景、音訊/動作和角色菜單。回放控制在右側。
...
**骨骼（Bone）**
：模型骨架中的一個點。動作用於動畫骨骼；物理模擬附加到骨骼上的部位。
