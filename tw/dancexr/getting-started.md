...
歡迎使用！本指南將帶你完成首次使用 DanceXR 的經驗，內容涵蓋從下載應用程式到載入模型並播放動作的整個流程。如果你是第一次使用 DanceXR，不用擔心：它被設計得直觀且易於使用。
...
| 平台 | 推薦版本 |
|----------|--------------|
| **Windows PC** | 選擇 **HD** 以兼顧畫質與平衡，**RT** 用於光線追蹤，或 **LW** 進行效能最佳化 |
| **Mac** | **HD** 版本 — 可透過 Steam 取得，但由於使用者基礎較小，最近尚未更新。 |
...
我們建議你在啟動 DanceXR 之前準備好內容庫，但你也可以透過將模型和動作檔案直接拖放到 DanceXR 視窗中來載入內容。目前你可以跳過此步驟，待你準備好加入自己的模型和動作時再回來。請注意，拖放功能僅適用於模型和動作，對於舞台、天空貼圖和材質等其他內容，你仍然需要在內容庫中放置它們。

DanceXR 會在 **內容庫** 資料夾中尋找模型、動作和其他內容。這就是你需要複製 PMX 和 XPS 模型檔案的地方。
...
開啟 DanceXR 後，你應該會在畫面底部看到一個選單列。如果沒有顯示，請點擊空白區域來切換 UI 狀態，直到它出現為止。
...
沒問題。你可以透過將模型檔案直接拖放到 DanceXR 視窗中即時載入。支援的格式包括 `.pmx`、`.xps`、`.mesh` 和 `.mesh.ascii`。只要將檔案拖入，它就會出現在角色選單中。

### 來自內容庫

點擊角色圖示並選擇「Load Model」（載入模型）。你會看到 `actors` 資料夾中所有模型的清單。預設情況下，新模型會取代目前舞台上選取的模型。如果你想新增而不是取代，請點擊模型名稱旁邊的「+」圖示。此功能僅適用於付費版本。
