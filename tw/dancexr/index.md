...
slideshows:
  load_play:
    ...
    - '/images/slideshows/load_play/4e0d4466664bf1e6fd615d3c69e03b74045bd0c7.jpg'
    - '/images/slideshows/load_play/e01294325adee543b4942b0aa5e917dfe7a67394.jpg'
    - '/images/slideshows/load_play/e2f4a1b6f2ab26b476aba086260cdf3dd89fcc59.jpg'
  motion:
  ...
  physics:
    ...
    - '/images/slideshows/physics/6b29c53917daff568a9ee75cfa0d62b6b4cadf79.jpg'
    - '/images/slideshows/physics/921e8f56935161e0e14e15d1dd0addb866f2b735.jpg'
  simulation:
    ...
    - '/images/slideshows/simulation/5da32fd1d351e71925eac92fff6c267204509241.jpg'
    - '/images/slideshows/simulation/f54ddf2a8664d98ba102af6dc65bd4e4b3adc159.jpg'
  dressing:
    ...
    - '/images/slideshows/dressing/90fbb9ac6e412cb83d0fcf3c743bfcd776473261.jpg'
    - '/images/slideshows/dressing/d4d311f45f38eb8210da93c3a97684eb44468b05.jpg'
  atmosphere:
  ...
  environment:
    ...
    - '/images/slideshows/environment/07c307fb4ce122f34e2e93152f783a4fc0316430.jpg'
    - '/images/slideshows/environment/35e7dbc7eff97344e4b4ebb5e68a0f3a74d085eb.jpg'
    - '/images/slideshows/environment/9a0cb02b7724fcc23dec1e3e8f9ec6c0c147adf9.jpg'
  camera:
...
大多數工具都需要您手動修復骨骼、調整姿勢，並為每個「模型-動作」組合微調設定。DanceXR 會為您自動完成這些步驟。

- **通用動作相容性** — VMD 和 BVH 動作無論是為 T Pose 還是 A Pose 製作，皆可在任何模型上正確播放
- **即時骨骼修復** — 自動修正常見的骨骼結構問題
- **全面支援 PMX 與 XPS** — 提供兩種格式的物理、材質和姿勢支援
...
DanceXR 中的角色不會只是呆站著。即使沒有任何設定，他們也會呼吸、眨眼並與您進行眼神交流。載入模型後，立刻就能感受到他們的存在。

- 當您進入視線時角色會看向您，當多個角色共用舞台時，他們會互相看著對方
- 程序化呼吸層疊在任何動作之上 — 無論角色在做什麼，總是處於活動狀態
- **自動舞蹈** — 根據音樂節拍和音量即時生成編舞
- 走秀、待機動作和動作序列讓場景保持動態，無需手動設置
...
DanceXR 擁有即時運行的完整物理模擬堆疊 — 頭髮、布料、軟體和裙子都會直接對動作做出反應。

{:.section-label}
物理系統

- **完全支援 PMX 模型物理** — 自動應用 PMX 模型的物理組件，並可透過強大的物理編輯器依模型自訂
- **頭髮、裙子與防重力物理** — XPS 模型的簡單設定，只需點擊幾下即可實現自然的物理模擬
...
- **布料模擬** — 具有撕裂、拖曳和網格轉布料轉換功能的即時平行多執行緒布料模擬
- **XPBD 粒子動力學** — 為達到最佳效率而平行執行的客製化進階粒子模擬
- **軟體模擬** — 透過 XPBD 進一步強化 XPS 物理工具
</div>
...
{:.section-label}
氛圍
- **粒子效果** — 具有完整物理控制的雪、閃光、雨和落葉
- **程序化天空** — 動態日夜轉換、星空、受風影響的體積雲
...
{:.section-label}
環境
- **HDR 環境貼圖** — 使用高動態範圍球形環境貼圖來獲得逼真的光影和背景
- **即時水體系統** — 反射、折射、漣漪和水下物理
- **舞台模型、道具、鏡子、雷射以及完整的水體系統**
</div>
...
- **自動相機** — 自動將所有演員保留在畫面中的智慧視角取景
- **自由飛行、環繞、第一人稱、固定 (演唱會模式)、一鏡到底**
- 支援具有完整插值曲線的 VMD 檔案相機動作
...
| 版本 | 專注焦點 |
|-------|-------|
| **RT** | 光線追蹤 —— 反射、全局照明、環境光遮蔽、接觸陰影 |
| **HD** | 高視覺品質，效能平衡 |
| **LW** | 幀率最佳化 —— 非常適合手機和 Quest |
...
透過 AI 驅動的語音聊天，您的角色能即時回應您。

- 連接到 **OpenAI**、本地 LLM (LM Studio、Ollama、Ooba) 或遠端 GPU 服務
- 內建 **TTS** 提供 900 多種聲音 —— 每個角色都可以有自己的聲音
- **自動對嘴** — 說話時臉部會有動畫反應
- **語音辨識** — 直接和他們說話，不僅限於打字
...
**Creator 版**會將螢幕解析度與幀率隔離開來以進行錄製。

- 以 60 fps 渲染 4K 影片 —— 即使在無法即時維持該幀率的 1080p 顯示器上也可以
- 輸出 **2D、立體 3D SBS、VR 180 和 VR 360** 影片
- 離線逐幀渲染 —— 確保每一幀都完美無瑕

[可在 Patreon 上獲取 →](/tw/dancexr/download){: .btn-ghost}
...
<div class="editions-grid" style="grid-template-columns: repeat(3, 1fr);">
  ...
  <div class="edition-card">
    ...
    <p class="edition-price">免費 — 僅限 PC</p>
    <div class="edition-divider"></div>
    <ul class="edition-features">
      <li>載入任意 PMX / XPS 模型</li>
      <li>播放任意 VMD / BVH 動作</li>
      <li>完整環境系統</li>
      <li class="locked">舞台上只有一名演員</li>
      <li class="locked">高階功能已鎖定</li>
    </ul>
    <a href="/tw/dancexr/download" class="edition-cta">免費下載</a>
  </div>

  <div class="edition-card featured">
    <p class="edition-tier">付費</p>
    <p class="edition-name">Pure / Pro</p>
    <p class="edition-price">Steam &amp; Itch.io 起價 $24.99</p>
    <div class="edition-divider"></div>
    <ul class="edition-features">
      <li>包含 Free 的所有功能</li>
      <li>多演員場景</li>
    ...
    <a href="/tw/dancexr/download" class="edition-cta">立即購買</a>
  </div>

  <div class="edition-card">
    <p class="edition-tier">Patreon</p>
    <p class="edition-name">Creator</p>
    <p class="edition-price">透過 Patreon 獲取 — 含搶先體驗</p>
    <div class="edition-divider"></div>
    <ul class="edition-features">
      <li>包含 Pro 的所有功能</li>
      <li>離線逐幀渲染</li>
...
<div class="editions-header" markdown="1">
  DanceXR 同樣支援 Android、iOS 和 Quest VR —— 相同的核心功能，針對您的裝置進行了最佳化。詳情請查看[下載頁面](/tw/dancexr/download)。
</div>
</section>
...
[查看所有版本說明 →](/tw/dancexr/releases){: .btn-ghost}
...
<div class="release-card">
  ...
  <div class="release-items">
    <div class="release-item">拖放文件載入</div>
    <div class="release-item">著色器最佳化 — 提升幀率</div>
    <div class="release-item">改進 VR 易用性</div>
    <div class="release-item">VMD2PNG 開源工具</div>
  </div>
  <a href="/tw/dancexr/releases/2026.3" class="btn-ghost">完整版本說明</a>
</div>
</div>
</section>
