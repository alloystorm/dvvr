...
## ## AI服务
有各种选项可用作AI服务，为您的角色提供智能。我们将在下面讨论每个选项的优缺点。
...
* 单击页面右上角的个人资料图标。
* 选择“查看API密钥”以打开API页面。
* 单击“创建新的秘密密钥”，并在显示时复制密钥。请记住，密钥**仅在此处显示一次**，您无法以后检索完整密钥，所以不要丢失它。
...
**优点：**
* 快速且简单
* 自由选择任何要运行的模型。甚至可以选择在本地GPU上无法运行的模型。
...
基本上就像用简单的文本告诉某人为你做某些事情。你可以修改默认模板并保存为不同的名称，以查看它如何影响聊天内容。例如，你可以在其中添加环境的描述，为聊天设置情景。

在DanceXR中，转到聊天设置->模板，选择你创建的模板。


### 角色
角色是根据角色模型的名称派生的。例如，“小春花束石斛发型B侧马尾”，“小春”将被解释为角色名称，其余的“花束石斛发型B侧马尾”将被用作她的服装描述。

如果角色是众所周知的，语言模型可能对角色有一些了解，所以有时它知道他们是谁以及他们的行为方式，尤其是当你使用OpenAI时。
...
然后将JSON文件放置在“chat/personas”文件夹中，它们将出现在角色设置的“人物角色”下拉菜单中。一旦你完成了这个操作，人物角色的描述将覆盖角色的描述。


### 聊天历史
每次生成新内容时，聊天历史都会发送给AI，以保持上下文。如果你想切换到不同的情景或话题，首先清除历史记录，这样AI就不会受到先前聊天上下文的影响。你也可以利用这一点来操纵环境并驱动聊天。例如，如果你在消息中描述了某件事发生了，AI将在那个上下文中继续。
...
### 附加语音
DanceXR使用一个名为Piper的TTS引擎。在这里，您可以听和下载其他语音模型以在DanceXR中使用。

https://rhasspy.github.io/piper-samples/

下载后，请将它们放入chat\voices\piper文件夹中。请注意，需要onnx和onnx.json两个文件。

### 语音管理器
内置的语音模型包含900多种不同的语音。默认情况下，我们只启用前20种语音。因为900种对用户来说太多了。要启用其他语音，请从聊天设置中转到Voice -> Voice Manager，从列表中选择一个语音，听一下，然后勾选“Selected”以允许将其添加到语音列表中。
