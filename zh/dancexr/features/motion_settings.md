...
## 选择
这是运动使用的骨骼和形变的列表。您可以使用开关来控制是否需要跳过该骨骼或形变。当您想要使用一个运动但不想使用它使用的所有骨骼或形变时，这很有用。

## T形或A形
运动设计用于与具有特定标准姿势的模型配合使用。传统上，这不能进行交叉匹配，但是使用DanceXR，您可以在任何模型上使用任何运动，而不考虑标准姿势。您只需要在此处选择正确的标准姿势，运动将被调整为与模型配合使用。

## 逆向运动学控制
DanceXR会自动确定运动是否使用逆向运动学。因此，大多数情况下，您可以将它们保留为自动，但如果逆向运动学系统存在任何问题，您可以在此处更改设置以使其正常工作。
...
速度：播放速度。默认情况下，所有运动都假定为30帧/秒。如果您的运动是60帧/秒，请将此速度设置为2。

## 运动平滑（v2026.2）
DanceXR 2026.2引入了一种先进的基于物理的运动平滑算法，增强了流畅性，同时模拟真实的加速度和阻尼效果，以重现自然的运动动态。

平滑功能默认对所有运动启用。您可以完全禁用平滑功能，或对单个关节进行精细调整以匹配您的偏好。与之前的算法相比，此版本还减少了延迟，带来更灵敏的响应体验。