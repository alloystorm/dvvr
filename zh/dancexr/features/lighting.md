...
**窗户** (*Window*) 子部分投射的矩形阴影网格，模拟穿过窗玻璃的流光。设置 *位置* (*Position*)、*底部* (*Bottom*) 和 *朝向* (*Orientation*) 来放置窗口，用 *宽度* (*Width*)、*高度* (*Height*)、*行数* (*Rows*) 和 *列数* (*Columns*) 定制尺寸；上述太阳控制决定了穿过窗户的光束方向。*天光* (*Sky Light*) 选项从同一方向添加柔和的补光，以补充阴影。
...
HDRP 自动曝光设置，用于控制摄影机如何适应场景亮度的变化。当禁用时，摄影机使用由全局暗度滑块驱动的固定曝光；启用时，它会根据场景亮度持续调整。
