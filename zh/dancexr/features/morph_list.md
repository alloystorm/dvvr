...
“形态列表”出现在任何加载的 **PMX** 模型的人格体菜单中。XPS 模型则使用 [Dressing system](/dancexr/features/optionals) 代替。
...
- **骨骼形态** — 微小的骨骼调整
- **材质形态** — 颜色或透明度变化（由 [dressing system](/dancexr/features/optionals) 使用）
- **顶点形态** — 直接的网格变形
- **组形态** — 以上各项的组合
...
只要您更改或重新加载模型，形态就会保持应用状态。一些形态（例如面部表情）将在 [Facial control](/dancexr/features/facial_control) 和 [Blink, breathing & eye contact](/dancexr/features/eyecontact) 激活时被其覆盖。
...
- 此处显示的 **材质形态** 通常与 [Dressing system](/dancexr/features/optionals) 提供的相同；无论从哪个位置应用，效果都是一样的。
