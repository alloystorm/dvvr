...
大多数工具都需要您手动修复骨骼、调整姿势，并为每个“模型-动作”组合微调设置。DanceXR 会为您自动完成这些操作。

- **通用动作兼容性** — VMD 和 BVH 动作无论是为 T Pose 还是 A Pose 制作，均可在任何模型上正确播放
- **即时骨骼修复** — 自动修正常见的骨骼结构问题
- **全面支持 PMX 与 XPS** — 提供两种格式的物理、材质和姿势支持
...
DanceXR 中的角色不会只是呆站着。即使没有任何设置，他们也会呼吸、眨眼并与您进行眼神交流。载入模型后，立刻就能感受到他们的存在。

- 当您进入视线时角色会看向您，当多个角色共用舞台时，他们会互相看着对方
- 程序化呼吸层叠在任何动作之上 — 无论角色在做什么，总是处于活动状态
- **自动舞蹈** — 根据音乐节拍和音量实时生成编舞
- 走秀、待机动作和动作序列让场景保持动态，无需手动设置
...
DanceXR 拥有实时运行的完整物理模拟堆叠 — 头发、布料、软体和裙子都会直接对动作做出反应。

{:.section-label}
物理系统

- **完全支持 PMX 模型物理** — 自动应用 PMX 模型的物理组件，并可通过强大的物理编辑器依模型进行自定义
- **头发、裙子与防重力物理** — XPS 模型的简单设置，只需点击几下即可实现自然的物理模拟
...
- **布料模拟** — 具有撕裂、拖拽和网格转布料转换功能的实时并行多线程布料模拟
- **XPBD 粒子动力学** — 为达到最佳效率而并行运行的定制高级粒子模拟
- **软体模拟** — 通过 XPBD 进一步增强 XPS 物理工具
</div>
...
{:.section-label}
氛围
- **粒子效果** — 具有完整物理控制的雪、闪光、雨和落叶
- **程序化天空** — 动态日夜转换、星空、受风影响的体积云
...
{:.section-label}
环境
- **HDR 环境贴图** — 使用高动态范围球形环境贴图来获得逼真的光影和背景
- **实时水体系统** — 反射、折射、涟漪和水下物理
- **舞台模型、道具、镜子、激光以及完整的水体系统**
</div>
...
- **自动相机** — 自动将所有演员保留在画面中的智能视角取景
- **自由飞行、环绕、第一人称、固定 (演唱会模式)、一镜到底**
- 支持具有完整插值曲线的 VMD 文件相机动作
...
通过 AI 驱动的语音聊天，您的角色能实时回应您。

- 连接到 **OpenAI**、本地 LLM (LM Studio、Ollama、Ooba) 或远程 GPU 服务
- 内置 **TTS** 提供 900 多种声音 —— 每个角色皆可拥有自己的声音
- **自动口型同步** — 说话时面部会有动画反应
...
**Creator 版**会将屏幕分辨率与帧率隔离开来进行录制。

- 以 60 fps 渲染 4K 视频 —— 即使在无法实时维持该帧率的 1080p 显示器上也可以
...
<div class="release-card">
  ...
  <div class="release-items">
    <div class="release-item">XPBD刚体物理系统</div>
    <div class="release-item">布娃娃&amp;软体模拟</div>
    <div class="release-item">全身IK &amp; 双足运动改进</div>
    <div class="release-item">并行BVH碰撞 — 性能提升</div>
  </div>
  <a href="/zh/dancexr/releases/2026.4" class="btn-ghost">完整版本说明</a>
</div>
</div>
</section>
