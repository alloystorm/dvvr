...
有关此处用到的术语（角色模型、选择圆盘、变换立方体、着装系统）的词汇表，请参阅 [Concepts & glossary](/dancexr/concepts)。
...
默认情况下，加载模型会**替换**当前选中的角色模型。点击模型名称旁边的 **+** 图标，则可以将其添加为额外的角色模型。多角色场景需要专业版（Pro）构建版本 — 请参阅 [Download & editions](/dancexr/download)。

### 加载选项

[Loader options](/dancexr/features/loader_options) 控制新角色模型加载的方式：缓存大小、纹理压缩、过渡效果、自动角色切换。
...
每个角色模型在其脚下都有一个黄色的 **选择圆盘**。点击它会打开角色模型菜单——这是所有关于角色模型的核心枢纽。有关完整分解，请参阅 [Actor menu & tools](/dancexr/features/actor_tools)。
...
- **变形列表** (仅 PMX) — [PMX 变形混合形状](/dancexr/features/morph_list)。
- **工具** (扳手和锤子图标) — 收藏、标签、[spectator](/dancexr/features/spectator_mode)、向上/向下移动、重置、[duplicate](/dancexr/features/actor_tools#tools-menu)、重新加载、[3D snapshot](/dancexr/features/snapshot_3d)、移除。
...
- [Formation](/dancexr/features/formation) — 按照图案定位角色模型（直线、网格、自定义）。顺序通过工具菜单中的 **Move Up** / **Move Down** 设置。
- [Actor playlist](/dancexr/features/actor_playlist) — 循环播放一段时间内的一系列模型，可选地与音乐同步。
- [Attach to actor](/dancexr/features/attach_to_actor) — 将一个角色模型或配件父级化到另一个角色模型上（携带物品、骑乘物、配对动作）。

使用 [Spectator mode](/dancexr/features/spectator_mode) 可以让模型留在舞台上，但将其排除在自动分配的运动和 formation 槽位之外。

[Global actor control](/dancexr/features/global_actor_control) 一次性为所有角色模型应用设置 — 当您希望所有加载的模型共享相同的物理骨架调校、着装或材质时非常有用。
...
- [3D snapshot](/dancexr/features/snapshot_3d) — 将当前姿势导出为 OBJ 文件，可用于其他 3D 工具。
- [Actor presets](/dancexr/features/actor_presets) — 保存角色模型的设置（材质、物理骨架、着装），以便稍后将相同的外观应用于相同的模型，或相似的模型。
- [Save scene](/dancexr/features/save_scene) 和 [scene bundle](/dancexr/features/scene_bundle) — 捕获整个场景，包括所有角色模型、运动、舞台和配置。
...
| 症状 | 查看位置 |
|---|---|
| 模型加载了，但所有内容都是白色 | [FAQ → Model loads but everything is white](/dancexr/faq) |
| 模型处于标准姿势时冻结 | [XPS bone mapper](/dancexr/features/bone_mapper), [Example bone structure](/dancexr/features/bones) |
| 漂浮、下沉、在地面上滑动 | [Feet adjustment](/dancexr/features/feet_adjustment) |
| 尺寸或位置错误 | [Scale & offset](/dancexr/features/scale_offset) |
| 头发/裙子/衣物不移动或抖动 | [Physics system](/dancexr/physics) |
| 特定模型异常 | [Actor troubleshooting](/dancexr/features/troubleshooting) |
| 应用程序级崩溃，无法启动 | [Troubleshooting](/dancexr/troubleshooting), [FAQ](/dancexr/faq) |
